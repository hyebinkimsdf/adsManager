...
import { resolveCampaignRefs, snapshotsToPromptJson } from "./context";
...
export function useLanguageModel(): UseLanguageModelResult {
  ...
  const ask = useCallback(
    async (message: string, campaigns: CampaignSnapshot[], turns: ChatTurn[] = []): Promise<AskResult> => {
      ...
      if (onDeviceReply) return { reply: resolveCampaignRefs(onDeviceReply, campaigns), engine: "on-device" };
