...
3. recommendations는 최대 3개, 서로 다른 campaignId를 고르세요.
   - kind가 "lower_bid"면 최근 7일 ROAS가 낮거나 지출 대비 문의가 적은 캠페인을 고르세요.
   - kind가 "raise_budget"이면 ROAS가 높고 성과가 좋은 캠페인을 고르세요.
   - kind가 "focus_target"이면 문의(전환)가 가장 많이 발생한 캠페인을 고르세요.
   - percent는 -30에서 30 사이 정수만 제안하세요(lower_bid는 음수, raise_budget은 양수). 근거 없는 과감한 변경은 피하세요.
...
4. spotlights는 최대 3개, recommendations와 무관하게 서로 다른 campaignId를 고르세요.
   ...
   - tag "rising": 최근 문의 추세가 상승 중인 캠페인 최대 1개(없으면 생략)
